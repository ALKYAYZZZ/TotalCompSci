...
public class Board {
	...
	public Board(Vector<Integer> _pits)
	{
		this.pits = new Vector<>(_pits);
		this.houses = (_pits.size() / 2) - 1;
		this.player1StoreIndex = houses;
		this.player2StoreIndex = _pits.size();
	...
	{
		Vector<Integer> temp = new Vector<Integer>(pits);
		for(int i=0; i<houses+1; i++) {
			pits.setElementAt(pits.elementAt(i), i+houses+1);
			pits.setElementAt(temp.elementAt(i+houses+1), i);
		}
	}
	
	public Boolean legalMove(Integer pitNum, Integer playerNum)
	{
		if (playerNum != 1 && playerNum != 2) return false;
		if (pitNum == player1StoreIndex || pitNum == player2StoreIndex) return false;
		if (player1StoreIndex < player2StoreIndex)
		{
			if (playerNum == 1)
			{
				return pitNum >= 0 && pitNum < player1StoreIndex;
			}
			else
			{
				return pitNum < player2StoreIndex && pitNum > player1StoreIndex;
			}
		}
		else
		{
			...
			{
				return pitNum < player2StoreIndex && pitNum >= 0;
			}
		}
	}
