...
class SocialNetworkServer final : public SocialNetwork::Service{
	private:
		...
		std::vector<string> return_followers(Person &person){
			std::vector<string> follows(person.followers_size());

			for(int i=0; i<follows.size(); ++i){
				follows[i] = person.followers(i);
			}
			return follows;
		}
		std::vector<string> return_following(Person &person){
			vector<string> following(person.following_size());

			for(int i=0; i<following.size(); ++i){
				following[i] = person.following(i);
			}
			return following;
		}
		void update_file(string name){
			...
			if(person_iterator == _people.end()){
				...
				std::fstream file(person_file, std::fstream::in | std::fstream::out);
				// for(int i=0; i<person.timeline_size(); ++i){
				// 	file.write(person.timeline(i));
				// 	file.write('\n');
				// }
				file.close();
			}
	...
	public:
		...
		Status sendCommand(ServerContext* context, const Commands* command, CommandReply* reply){
			string _personname = command->person();
			...
			if(type == socialnetwork::Commands_Choices_UNKNOWN){
				...
				reply->set_return_val(follow(_personname, _args));
			}
			else if(type == socialnetwork::Commands_Choices_UNFOLLOW){
				reply->set_return_val(unfollow(_personname,_args));
			}
			else if(type == socialnetwork::Commands_Choices_LIST){
				reply->set_return_val(list(_personname));
			}
			else if(type == socialnetwork::Commands_Choices_TIMELINE){
				reply->set_return_val(timeline(_personname));
			}
			return Status::OK;
		}
		Status connectUser(ServerContext* context, const Person* person, CommandReply* reply){
			string _personname = person->name();
			if (_people.find(_personname) == _people.end()) {
				string person_file = path + _personname + "-info";
				if(existing_file(person_file)){

				}
				else{
					Person new_person;
					new_person.set_name(_personname);
					_people.emplace(_personname, new_person);
				}
...
/*	else if (argc < 2){
		cerr << "Port number unspecified, please try again." << endl;
		return EXIT_FAILURE;
	}
	else{
