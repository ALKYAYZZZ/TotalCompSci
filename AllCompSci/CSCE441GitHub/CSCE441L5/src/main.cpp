...
static void mouse_callback(GLFWwindow *window, int button, int action, int mods)
{
	...
	if(action == GLFW_PRESS) {
		...
		newPt[0] = posX*(((xMaxWorld - xMinWorld))/width) + xMinWorld;
		newPt[1] = posY*(((yMaxWorld - yMinWorld))/height) + yMinWorld;
...
static void init()
{
	...
	colBuf.push_back(0.0f);
	colBuf.push_back(0.8f);
	colBuf.push_back(0.0f);
	colBuf.push_back(0.8f);
	colBuf.push_back(0.8f);
	colBuf.push_back(0.0f);
