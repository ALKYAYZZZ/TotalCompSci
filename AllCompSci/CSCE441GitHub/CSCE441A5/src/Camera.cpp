...
void Camera::applyFirstPersonMatrix(std::shared_ptr<MatrixStack> MV, glm::mat4 heli_matrix, float x, float y, float z) const {
	//Translate and rotate the camera to behind the helicopter and multiply the inverted helicopter matrix by the model
	//view matrix
	MV->translate(x, y, z);
	MV->rotate(-1.57f, 0.0f, 1.0f, 0.0f);
	MV->multMatrix(glm::inverse(heli_matrix));
}

glm::vec3 Camera::get_translations() {
	return translations;
}
