...
void render()
{
	...
	if (cps.size() >= 4) {
		mat4 B;
		if (type == BEZIER) {
			B[0] = vec4(1.0f, 0.0f, 0.0f, 0.0f);
			B[1] = vec4(-3.0f, 3.0f, 0.0f, 0.0f);
			B[2] = vec4(3.0f, -6.0f, 3.0f, 0.0f);
			B[3] = vec4(-1.0f, 3.0f, -3.0f, 1.0f);
		}
		else if (type == CATMULL_ROM) {
			B[0] = vec4(0.0f, 1.0f, 0.0f, 0.0f);
			B[1] = vec4(-0.5f, 0.0f, 0.5f, 0.0f);
			B[2] = vec4(1.0f, -2.5f, 2.0f, -0.5f);
			B[3] = vec4(-0.5f, 1.5f, -1.5f, 0.5f);
		}
		else if (type == BSPLINE) {
			B[0] = vec4(.167f, .667f, .167f, 0.0f);
			B[1] = vec4(-0.5f, 0.0f, 0.5f, 0.0f);
			B[2] = vec4(0.5f, -1.0f, 0.5f, 0.0f);
			B[3] = vec4(-.167f, 0.5f, -0.5f, .167f);
		}
		// Fill in G column by column
		mat4 G;
		int difference = cps.size() - 1;
		G[0] = vec4(cps[difference - 3], 0.0f);
		G[1] = vec4(cps[difference - 2], 0.0f);
		G[2] = vec4(cps[difference - 1], 0.0f);
		G[3] = vec4(cps[difference], 0.0f);
		total.push_back(G);
		
		/*
		if (total.size() != 0) {
			for (int i = 0; i < total.size(); i++) {
				glVertex3f(total[i].x, total[i].y, total[i].z);
			}
		}
		*/
