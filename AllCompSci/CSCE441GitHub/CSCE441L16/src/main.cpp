...
void render()
{
	...
	glUniform1f(progTex->getUniform("g"), choose);
	if (!keyToggles[(unsigned)'g']) {
		shape->toWorld();
	}
	shape->draw(progTex->getAttribute("aPos"), progTex->getAttribute("aTex"), progTex->getAttribute("posLocal"), progTex->getAttribute("tileIndex"));
	texture->unbind();
	progTex->unbind();
