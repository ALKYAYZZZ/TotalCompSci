...
void* worker_thread_function(void* arg) {
     ...
     */
	 ...
	 std::string s = pass_param->worker->send_request("newthread");
...
int main(int argc, char * argv[]) {
	...
	if (pid > 0) {
		...
		SafeBuffer *request_buffer = new SafeBuffer(n);
...
/*--------------------------------------------------------------------------*/
		...
		for(int i = 0; i < w; i++){
			...
			pthread_create(&worker[i], NULL, worker_thread_function, (void*)fun);
		}
		/*
        for(int i = 0; i < n; ++i) {
		...
		*/
		...
		jane_frequency_count = fun->jane;
		joe_frequency_count = fun->joe;
		john_frequency_count = fun->john;
