...
struct holper{
	NetworkRequestServer *worker;
	NetworkRequestServer *thread;
	std::vector<int> john;
	std::vector<int> jane;
	std::vector<int> joe;
	bounded_buffer *buff;
...
void* worker_thread_function(void* arg) {
	struct holper* pass_param = (holper*)arg;
	 ...
	 NetworkRequestServer *workerChannel = pass_param->thread;
...
void* event_thread_function(void* arg){
	...
	vector<NetworkRequestServer*> chans;
	//NetworkRequestServer * con = new NetworkRequestServer("control", NetworkRequestServer::CLIENT_SIDE);
	
	for(int i = 0; i < pass_param->w; i++){
		//std::string s = con->send_request("newthread");
		//cerr << atoi(p);
		NetworkRequestServer * chan = new NetworkRequestServer(h.c_str(), atoi(p));
	...
	while(true){
		...
		selects = select(file_max + 1, &rfd, NULL, NULL, NULL);
		//std::cerr << selects;
		if(selects){
			for(int i = 0; i < pass_param->w; i++){
...
int main(int argc, char * argv[]) {
	...
	//if (pid > 0) {
        ...
        //NetworkRequestServer *chan = new NetworkRequestServer("control", NetworkRequestServer::CLIENT_SIDE);
		...
		*/
		...
		//for(int i = 0; i < w; i++){
			...
			//fun->thread = new NetworkRequestServer(s, NetworkRequestServer::CLIENT_SIDE);
